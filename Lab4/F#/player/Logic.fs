...
module Logic =
    ...
    let updateInput (current: GameObject option) grid scoreText (tetraminos: GameObject[]) spawner score =
        let obj, score =
            match current with
            | Some c ->
    ...
    let gameLoop gameState scoreText (tetraminos: GameObject[]) spawner (callback: UpdateCallbackOptional) =
        Debug.Log "Game Loop"
        ...
        | None ->
            let finalCurrent, newScore =
                updateInput updatedGameState.Current grid scoreText tetraminos spawner updatedGameState.Score
